...
Scarlet.prototype.reset = function() {
  this.quit=false;
  this.mem=[];
  this.lastchoice=[];
...
Scarlet.prototype._init = function() {
  ...
  Scarlet.prototype.pres={};
  Scarlet.prototype.posts={};

  if ((this.elizaPres) && (this.elizaPres.length)) {
    var a=new Array();
    for (var i=0; i<this.elizaPres.length; i+=2) {
      a.push(this.elizaPres[i]);
      Scarlet.prototype.pres[this.elizaPres[i]]=this.elizaPres[i+1];
    }
    Scarlet.prototype.preExp = new RegExp('\\b('+a.join('|')+')\\b');
  } else {
    // default (should not match)
    Scarlet.prototype.preExp = /####/;
    Scarlet.prototype.pres['####']='####';
  }
  ...
  if ((this.elizaPosts) && (this.elizaPosts.length)) {
    ...
    for (var i=0; i<this.elizaPosts.length; i+=2) {
      ...
      Scarlet.prototype.posts[this.elizaPosts[i]]=this.elizaPosts[i+1];
    }
    Scarlet.prototype.postExp = new RegExp('\\b('+a.join('|')+')\\b');
  } else {
    // default (should not match)
    Scarlet.prototype.postExp = /####/;
    Scarlet.prototype.posts['####']='####';
  }
  ...
  // done
  Scarlet.prototype._dataParsed=true;
};

Scarlet.prototype._sortKeywords = function(a, b) {
  // sort by rank
  if (a[1]>b[1]) return -1;
...
Scarlet.prototype.transform = function(text) {
  let rpl='';
  this.quit=false;
  // unify text string
...
Scarlet.prototype._execRule = function(k) {
...
Scarlet.prototype._postTransform = function(s) {
  // final cleanings
  s=s.replace(/\s{2,}/g, ' ');
  s=s.replace(/\s+\./g, '.');
...
Scarlet.prototype._getRuleIndexByKey = function(key) {
  for (let k=0; k<this.elizaKeywords.length; k++) {
...
Scarlet.prototype._memSave = function(t) {
  this.mem.push(t);
  if (this.mem.length>this.memSize) this.mem.shift();
};

Scarlet.prototype._memGet = function() {
  if (this.mem.length) {
